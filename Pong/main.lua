...
function love.load()
    ...
    player1 = Paddle(10, 30, 2, 60)
    player2 = Paddle(VIRTUAL_WIDTH - 10, VIRTUAL_HEIGHT - 30, 2, 60)
...
function updateAI(dt)
    -- Calculate the difference between the AI paddle's y-coordinate and the ball's y-coordinate
    local paddleCenter = player2.y + player2.height / 2
    local ballCenter = ball.y + ball.height / 2
    local paddleToBallDifference = paddleCenter - ballCenter

    -- Move the AI paddle based on the difference
    if paddleToBallDifference < -10 then
        player2.dy = AI_PADDLE_SPEED
    elseif paddleToBallDifference > 10 then
        player2.dy = -AI_PADDLE_SPEED
    else
        player2.dy = 0
    end
